...
`)
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS mint_records (
        ...
        vaultId VARCHAR(100) NOT NULL,
        vaultChain VARCHAR(100) NOT NULL,
        userVaultChainAddress VARCHAR(255) NOT NULL,
        amount DECIMAL(36, 18) NOT NULL,
        utxo VARCHAR(200) NOT NULL,
    ...
    await connection.query(`
        ...
        amount DECIMAL(36, 18) NOT NULL,
        burnTxHash VARCHAR(200) NOT NULL UNIQUE,
        nativeRecipientAddress VARCHAR(255) NOT NULL,
        nativeTransactionId VARCHAR(200),
        createdAt DATETIME DEFAULT CURRENT_TIMESTAMP,
        updatedAt DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        status ENUM('pending', 'processing', 'completed', 'failed') NOT NULL DEFAULT 'pending',
...
export class DatabaseService {
  ...
  static async updateMintRecord(
    native_hash: string,
    updates: Partial<MintRecord>
  ): Promise<MintRecord | null> {
    const pool = getPool()

    const updateQuery = `
      UPDATE mint_records
      SET status = ?, minting_hash = ?
      WHERE native_hash = ?;
    `
    const updateValues = [updates.status, updates.minting_hash, native_hash]
    ...
    const selectQuery = `
      ...
      LIMIT 1;
    `
    const [rows] = await pool.query(selectQuery, [native_hash])

    return (rows as MintRecord[])[0] || null
  }
