...
const connectionConfig = {
  host: process.env.DB_HOST || "localhost",
  port: process.env.DB_PORT ? Number.parseInt(process.env.DB_PORT) : 25060,
  user: process.env.DB_USER || "doadmin",
  password: process.env.DB_PASSWORD || "",
  multipleStatements: process.env.DB_MULTIPLE_STATEMENTS === "true",
}
...
export const initDB = async () => {
  try {
    ...
    // Create mint_records table if it doesn't exist
    await connection.query(`
      CREATE TABLE IF NOT EXISTS mint_records (
        ...
        amount DECIMAL(36, 18) NOT NULL,
        utxo VARCHAR(200) NOT NULL,
        native_hash VARCHAR(200) NOT NULL,
        minting_hash VARCHAR(200),
        verified BOOLEAN NOT NULL DEFAULT FALSE,
        whitelisted BOOLEAN NOT NULL DEFAULT FALSE,
        proof TEXT NOT NULL,
        createdAt DATETIME DEFAULT CURRENT_TIMESTAMP,
        updatedAt DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        status ENUM('pending', 'verified', 'whitelisted', 'minted', 'rejected','completed') NOT NULL DEFAULT 'pending',
        INDEX idx_evmAddress (evmAddress),
        INDEX idx_native_hash (native_hash),
        INDEX idx_minting_hash (minting_hash),
        INDEX idx_verified (verified),
        INDEX idx_status (status),
        INDEX idx_createdAt (createdAt)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
    `)
    console.log("mint_records table created or already exists")
...
export class DatabaseService {
  static async createMintRecord(record: Omit<MintRecord, "id" | "createdAt" | "updatedAt">): Promise<MintRecord> {
    const pool = getPool()
    const id = `mint_${uuidv4()}`
    const now = new Date()
    const createdAt = now.toISOString().slice(0, 19).replace("T", " ")
    ...
    console.log("Creating mint record with ID:", id)
    ...
    try {
      await pool.query(
        `INSERT INTO mint_records (
          id, evmAddress, evmChain, evmChainId, vaultId, vaultChain, userVaultChainAddress,
          amount, utxo, native_hash, minting_hash, verified, whitelisted, proof, createdAt, updatedAt, status
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          id,
          evmAddress,
      ...
      console.log("Successfully created mint record:", id)
  ...
  static async getMintRecordsByEvmAddress(evmAddress: string): Promise<MintRecord[]> {
    const pool = getPool()
    console.log("Fetching mint records for address:", evmAddress)

    try {
      const [rows] = await pool.query(
        `SELECT * FROM mint_records WHERE LOWER(evmAddress) = LOWER(?) ORDER BY createdAt DESC`,
        [evmAddress],
      )
      console.log(`Found ${(rows as any[]).length} mint records for address ${evmAddress}`)
      return rows as MintRecord[]
    } catch (error) {
      console.error("Error fetching mint records by address:", error)
      throw error
    }
  }

  static async getMintRecordByNativeHash(native_hash: string): Promise<MintRecord | null> {
    const pool = getPool()
    console.log("Fetching mint record by native hash:", native_hash)

    try {
      const [rows] = await pool.query(`SELECT * FROM mint_records WHERE native_hash = ? LIMIT 1`, [native_hash])
      const result = (rows as MintRecord[])[0] || null
      console.log("Mint record found:", !!result)
      return result
    } catch (error) {
      console.error("Error fetching mint record by native hash:", error)
      throw error
    }
  }

  static async updateMintRecord(id: string, updates: Partial<MintRecord>): Promise<MintRecord | null> {
    const existing = await this.getMintRecordById(id)
    if (!existing) {
      console.log("Mint record not found for update:", id)
      return null
    }
    ...
    console.log("Updating mint record:", id)

    try {
      await pool.query(
        `UPDATE mint_records SET
          evmAddress = ?, evmChain = ?, evmChainId = ?, vaultId = ?, vaultChain = ?,
          userVaultChainAddress = ?, amount = ?, utxo = ?, native_hash = ?, minting_hash = ?, 
          verified = ?, whitelisted = ?, proof = ?, updatedAt = ?, status = ?, requestType = ?
        WHERE id = ?`,
        [
          updated.evmAddress,
          updated.evmChain,
      ...
      console.log("Successfully updated mint record:", id)
      return updated
    } catch (error) {
      console.error("Error updating mint record:", error)
      throw error
    }
  }

  static async getAllMintRecords(): Promise<MintRecord[]> {
    const pool = getPool()
    console.log("Fetching all mint records")

    try {
      const [rows] = await pool.query(`SELECT * FROM mint_records ORDER BY createdAt DESC`)
      console.log(`Found ${(rows as any[]).length} total mint records`)
      return rows as MintRecord[]
    } catch (error) {
      console.error("Error fetching all mint records:", error)
      throw error
    }
  }

  static async getMintRecordById(id: string): Promise<MintRecord | null> {
    const pool = getPool()
    console.log("Fetching mint record by ID:", id)

    try {
      const [rows] = await pool.query(`SELECT * FROM mint_records WHERE id = ? LIMIT 1`, [id])
      const result = (rows as MintRecord[])[0] || null
      console.log("Mint record found by ID:", !!result)
      return result
    } catch (error) {
      console.error("Error fetching mint record by ID:", error)
      throw error
    }
  }

  static async getLatestMintRecordByAddress(evmAddress: string, vaultId?: string): Promise<MintRecord | null> {
    const pool = getPool()
    console.log("Fetching latest mint record for address:", evmAddress, "vaultId:", vaultId)

    let query = `SELECT * FROM mint_records WHERE LOWER(evmAddress) = LOWER(?)`
    const params: any[] = [evmAddress]
    ...
    try {
      const [rows] = await pool.query(query, params)
      const result = (rows as MintRecord[])[0] || null
      console.log("Latest mint record found:", !!result)
      return result
    } catch (error) {
      console.error("Error fetching latest mint record:", error)
      throw error
    }
  }
