...
export default function EarnDepositPage() {
   ...
   const params = useParams(); // returns an object with dynamic route params



const assets = [
    { id: 'doge', asset: 'tDOGE', icon: 'Ð',  },
    { id: 'litecoin', asset: 'tLTC', icon: 'Ł',  },
    { id: 'bitcoin_cash', asset: 'tBCH', icon: '₿', },
    ];
      const { id } = useParams(); // 'litecoin', for example

  const selectedAsset = assets.find(a => a.id === id)?.asset;
    const handleAction = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 md:p-8 animate-fade-in">
             <Link href="/earn" className="text-muted-foreground hover:text-foreground mb-6 flex items-center gap-2">
                &larr; Back to Earn
            </Link>
            <h2 className="text-3xl font-bold text-foreground mb-2">Earn</h2>
            <p className="text-muted-foreground mb-8">Deposit your {selectedAsset} into the Stability Pool to earn rewards.</p>
            ...
            <div className="bg-card/70 border rounded-2xl p-6 shadow-xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-secondary/50 p-4 rounded-lg">
                        ...
                        <p className="text-2xl font-bold text-foreground">25.7M {selectedAsset}</p>
                    </div>
                    <div className="bg-secondary/50 p-4 rounded-lg">
                        <p className="text-sm text-muted-foreground">Your Deposit</p>
                        <p className="text-2xl font-bold text-foreground">{currentDeposit.toLocaleString()} {selectedAsset}</p>
                    </div>
                </div>
                    ...
                    {mode === 'deposit' ? (
                        <div className="animate-fade-in-sm">
                            ...
                            <div className="relative">
                                ...
                                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground font-bold">{selectedAsset}</span>
                            </div>
