...
useEffect(() => {
  const connect = async () => {
    ...
    try {
      ...
      setPageError("Failed to read from contracts. Ensure correct network and contract setup.")
    }
  }
...
useEffect(() => {
  const fetchState = async () => {
    if (
      ...
      pool.contractAddress === '0x0000000000000000000000000000000000000000' ||
      pool.stakingTokenAddress === '0x0000000000000000000000000000000000000000'
    ) {
      setIsLoading(false)
      return
    }
  ...
  fetchState()
}, [address, isCorrectNetwork, pageError, pool.contractAddress, pool.stakingTokenAddress])
...
const handleApprove = async () => {
  if (!depositAmount || Number.parseFloat(depositAmount) <= 0) {
    toast({ title: 'Invalid Amount', description: 'Please enter a valid amount to approve', variant: 'destructive' })
    return
  }
  if (!isCorrectNetwork) {
  ...
  try {
    ...
    setShowModal(true)
    setModalContent({ title: 'Approving Token', description: `Please confirm approval for ${pool.asset}...`, isSuccess: false })
    ...
    setModalContent({ title: 'Approval Submitted', description: 'Waiting for confirmation...', isSuccess: false })
   await waitForTransactionReceipt(config, {
  hash: hash as `0x${string}`,
  confirmations: 1,
})

    toast({ title: 'Success', description: `Approved ${depositAmount} ${pool.asset}` })
    setModalContent({ title: 'Approval Successful', description: `Approved ${depositAmount} ${pool.asset}`, isSuccess: true, txHash: hash })
...
const handleDeposit = async () => {
  ...
  if (amountBigInt > tokenAllowance) return toastErr('Insufficient allowance')

  try {
    setIsProcessing(true)
    setShowModal(true)
    setModalContent({ title: 'Deposit Pending', description: 'Confirm the transaction...', isSuccess: false })
    ...
    setModalContent({ title: 'Submitted', description: 'Waiting for confirmation...', isSuccess: false })
    await waitForTransactionReceipt(config, {
  hash: hash as `0x${string}`,
  confirmations: 1,
})

    toast({ title: 'Success', description: `Deposited ${depositAmount} ${pool.asset}` })
    setModalContent({ title: 'Deposit Successful', description: `Deposited ${depositAmount} ${pool.asset}`, isSuccess: true, txHash: hash })
    setDepositAmount('')
...
const handleWithdraw = async () => {
  ...
  if (amountBigInt > stakedBalance) return toastErr('Insufficient staked balance')

  try {
    setIsProcessing(true)
    setShowModal(true)
    setModalContent({ title: 'Withdraw Pending', description: 'Confirm withdrawal...', isSuccess: false })
    ...
    toast({ title: 'Success', description: `Withdrew ${withdrawAmount} ${pool.asset}` })
    setModalContent({ title: 'Withdraw Successful', description: `Withdrew ${withdrawAmount} ${pool.asset}`, isSuccess: true, txHash: hash })
    setWithdrawAmount('')
...
function handleError(type: string, err: any) {
  ...
  if (err?.message?.includes('user rejected')) message = `${type} was cancelled`
  else if (err?.message?.includes('insufficient funds')) message = 'Insufficient funds for gas'
  else if (err?.message) message = err.message

  setModalContent({ title: `${type} Failed`, description: message, isSuccess: false })
